...
def select_folder():
    ...
    for filename in os.listdir(folder_path):
        ...
        # Pilot-based filtering
        if pilot_value == 0 and 'pilot' in filename.lower():
            continue
        if pilot_value == 2 and 'pilot' not in filename.lower():
            continue

        # Read CSV
        file_path = os.path.join(folder_path, filename)
        try:
            df = pd.read_csv(file_path, header=0, skiprows=range(1, row_skip))
        except pd.errors.EmptyDataError:
            continue  # skip completely empty files

        if df.empty:
            continue  # skip files that only contain headers or no data
...
def run_analysis():
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.csv'):
            ...
            # Skip if the dataframe has no columns or rows
