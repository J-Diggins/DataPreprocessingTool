...
pilot_value = 0
widgets = {}
ignore_strings = ["expName","participant","OS","session","psychopyVersion","frameRate","date",".x",".y",".started",".stopped","thisIndex","Unnamed","leftButton","midButton","rightButton"]
...
def select_folder():
    global folder_path, column_names, pilot_value, c2
    folder_path = ''
    column_names = []
    ...
    if has_piloting:
        ...
        pilot_dialog.transient(root)
        pilot_dialog.grab_set()  
...
def shift_rows():
    ...
    col_dialog.geometry("300x300")
    
    tk.Label(col_dialog, text="Select columns to shift data in:", font=("Arial", 10, "bold")).pack(pady=5)
    
    # Start column
    tk.Label(col_dialog, text="Start Column").pack(pady=(10, 0))
    start_col_var = tk.StringVar()
    
    start_combo = ttk.Combobox(
        col_dialog,
        textvariable=start_col_var,
        values=column_names,
        state="readonly"
    )
    ...
    # End column
    tk.Label(col_dialog, text="End Column").pack(pady=(10, 0))
    end_col_var = tk.StringVar()
    
    end_combo = ttk.Combobox(
        col_dialog,
        textvariable=end_col_var,
        values=column_names,
        state="readonly"
    )
    ...
    def confirm_columns():
        ...
        start_col = start_col_var.get()
        end_col = end_col_var.get()
    
        if not start_col or not end_col:
            messagebox.showerror(
                "Error",
                "Please select both a start and end column."
            )
            return
    
        start_idx = column_names.index(start_col)
        end_idx = column_names.index(end_col)
    
        # Allow selection in either direction
        if start_idx > end_idx:
            start_idx, end_idx = end_idx, start_idx
    
        # Get all columns in the range
        selected_columns = column_names[start_idx:end_idx + 1]
...
def add_row():
    ...
    c2_new = ttk.Combobox(b_frame,
        state="readonly",
        values=column_names
    )
    c2_new.place(x=50, y=50)
    c2_new.grid(row=row_counter, column=1)
    ...
    widgets[f"{row_counter}"] = {"buttons": [b, b1, b2], "combos": [c1, c2_new]}
...
def main():
    global root, t_frame, b_frame, path_label, stat_label, excol_label, cond_label, c1, c2, b, b1, b2, row_counter, widgets
